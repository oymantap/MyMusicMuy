...
override fun onCreateView(
    ...
    rvLyrics =
        view.findViewById(R.id.rvLyrics)

    tvNoLyrics =
        view.findViewById(R.id.tvNoLyrics)
    ...
    fun updateLyricsHighlight(currentMs: Long) {
        ...
        if (
            ...
            val offset =
                (rvLyrics.height / 2) - 100

            (
                rvLyrics.layoutManager
    ...
    fun loadLyrics(songUri: Uri?) {
        ...
        // Biar gak reload terus tiap updateUI()
        if (songUri == lastLoadedUri) return

        lastLoadedUri = songUri
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            ...
            val tempLyrics =
                withContext(Dispatchers.IO) {
                    ...
                    parseLrcFile(songUri)
                }

            if (
                tempLyrics != null &&
                tempLyrics.isNotEmpty()
            ) {
    ...
    private fun parseLrcFile(
        ...
        try {
            ...
            val fullFileName =
                songFile?.name ?: return null

            val lrcName =
                fullFileName.substringBeforeLast(".") +
                        ".lrc"
            ...
            val lrcFile =
                rootTree?.findFile(lrcName)
                    ?: return null

            context.contentResolver
                .openInputStream(lrcFile.uri)
                ?.use { inputStream ->
